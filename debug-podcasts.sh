#!/bin/bash
# Build, deploy, and debug MediaDash Android app - podcast data flow logs
# Tracks BLE command flow between golang_ble_client and MediaDash Android
#
# Shows: Podcast list requests, episode requests, episode playback, and BLE transmission
...
show_usage() {
    echo "Usage: $0 [-s] [-l] [-v] [-g]"
    echo ""
    echo "Options:"
    echo "  -s    Skip build, just install and run"
    echo "  -l    Logs only mode (don't build/install, just stream logs)"
    echo "  -v    Verbose mode (show all podcast logs, not just key events)"
    echo "  -g    Also show Go client logs (requires SSH to CarThing)"
    echo ""
    echo "Log Tags Monitored:"
    echo "  PODCAST     - High-level podcast operations (requests, responses)"
    echo "  GattServer* - BLE command reception and transmission"
    echo "  MediaRepo*  - Podcast data fetching and playback"
    echo "  PodcastAudio- Podcast audio playback events"
    echo ""
    echo "Examples:"
    echo "  $0           # Full build, deploy, and debug"
    echo "  $0 -l        # Just stream podcast logs"
    echo "  $0 -l -v     # Verbose podcast logs"
...
print_header "MediaDash Podcast Debug Session"
...
# Function to colorize podcast logs
colorize_podcast_logs() {
    while read -r line; do
        # Request events (incoming from BLE)
        if echo "$line" | grep -qiE "(request_podcast_list|request_recent_episodes|request_podcast_episodes|podcast.*request.*received)"; then
            echo -e "${MAGENTA}$line${NC}"
        # Response/transmission events
        elif echo "$line" | grep -qiE "(sending.*podcast|transmit|notify.*podcast|response.*episodes|returning.*podcast)"; then
            echo -e "${GREEN}$line${NC}"
        # Playback events
        elif echo "$line" | grep -qiE "(play_podcast_episode|playing.*podcast|podcast.*playback|episode.*play)"; then
            echo -e "${WHITE}$line${NC}"
        # Success events
        elif echo "$line" | grep -qiE "(success|✓|✅|complete)"; then
            echo -e "${GREEN}$line${NC}"
        # Warning events
        elif echo "$line" | grep -qiE "(warning|⚠️|not found|no.*podcast)"; then
        ...
        # BLE command events
            ...
            echo -e "${CYAN}$line${NC}"
        # Data counts/stats
        elif echo "$line" | grep -qiE "([0-9]+\s*(podcast|episode|channel))"; then
            echo -e "${BLUE}$line${NC}"
        # Pagination info
        elif echo "$line" | grep -qiE "(offset|limit|hasmore|pagination)"; then
            echo -e "${GRAY}$line${NC}"
        else
            echo "$line"
        fi
...
stream_logs() {
    ...
    if [ "$VERBOSE" = true ]; then
        # Verbose: All podcast-related tags at verbose level
        adb -s "$DEVICE_ID" logcat -v time \
            ...
            PodcastAudio:V \
            PodcastPlayerManager:V \
            *:S | colorize_podcast_logs
    else
        # Normal: Key podcast events only
        adb -s "$DEVICE_ID" logcat -v time \
            PODCAST:I \
            GattServerService:D \
            GattServerManager:D \
...
if [ "$LOGS_ONLY" = true ]; then
    print_header "Streaming Podcast Data Flow Logs"
    echo -e "${YELLOW}Press Ctrl+C to stop${NC}"
    echo ""
    adb -s "$DEVICE_ID" logcat -c

    echo "Legend:"
    echo -e "  ${MAGENTA}REQUEST   = Incoming podcast data request from BLE${NC}"
    echo -e "  ${GREEN}RESPONSE  = Outgoing podcast data via BLE${NC}"
    echo -e "  ${WHITE}PLAYBACK  = Episode playback commands${NC}"
    echo -e "  ${CYAN}BLE       = BLE characteristic operations${NC}"
    echo -e "  ${BLUE}DATA      = Data counts and statistics${NC}"
...
print_header "Streaming Podcast Data Flow Logs"
echo ""
echo -e "${YELLOW}Watching for podcast data flow events...${NC}"
echo -e "${YELLOW}Press Ctrl+C to stop${NC}"
echo ""
echo "Legend:"
echo -e "  ${MAGENTA}REQUEST   = Incoming podcast data request from BLE${NC}"
